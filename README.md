...
**Important Tip:** If during the tutorial, you do any change in cloudformation, template or paramters file, make sure to `amplify env checkout` before doing `amplify push`. Otherwise, the cli doesn't detect change during `amplify status`.
